      ...
      <van-tabs v-model="active" color="#000" background="#fff" :sticky="true" @change="tab">
        ...
        <van-tab title="待我处理">
          <Mescroll-vue ref="mescroll" :up="mescrollUp" @init="mescrollInit" class="mescroll">
            ...
            <div class="minBox" v-for="(item,i) in list" :key="i" @click="sign(item)">
              <div class="flex col">
                <span>事件编号</span>
              ...
              <div class="flex col">
                <span>申请部门</span>
                <span>{{item.applyDept}}</span>
              </div>
              <div class="flex col">
                <span>起止时间</span>
                <span>{{item.applyTime|timeFilter('ymdhm')}}</span>
              </div>
            </div>
          </Mescroll-vue>
        </van-tab>
        <van-tab title="我已处理">
          <Mescroll-vue ref="mescroll" :up="mescrollUp" @init="mescrollInit" class="mescroll">
            ...
            <div class="minBox" v-for="(item,i) in list1" :key="i" @click="sign2(item.id)">
              <div class="flex col">
                <span>事件编号</span>
                <span>{{item.eventCode}}</span>
              </div>
      ...
      this.keepAxios
        .get(this.baseUrl.checkList + '/rest/appUser/applyJoinCompany', {
          params: {
            ...
            limit: page.size,
            uid: localStorage.getItem('uid'),
            status: this.active //0：待我处理 1: 我已处理
          }
        })
        .then(data => {
          console.log(data)
