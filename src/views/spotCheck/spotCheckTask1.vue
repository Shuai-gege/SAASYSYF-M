<template>
  <div class="box">
    ...
    <!-- 拍照 -->
    <div class="paiZhao">
      <van-uploader v-model="fileList" :max-count="3" :after-read="afterRead" @delete="remove" />
    </div>
  </div>
</template>
...
export default {
  ...
  mounted() {
    ...
    sessionStorage.setItem('beacons', false)
    localStorage.setItem('faceCheck', '0')
  },
  methods: {
    // 压缩图片
    ...
    onClickLeft() {
      this.$router.push('/spotCheckDetails')
    },
    ad() {
      console.log('aaa')
    },
        ...
        this.keepAxios
          ...
          .then(data => {
            ...
            if (data.data.code == 20000) {
              ...
              console.log('下')
              console.log(data.data.data.deviceInfoBean.deviceName)
              console.log('上')
              this.$router.push({
