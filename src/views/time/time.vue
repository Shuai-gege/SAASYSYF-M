<template>
  <div class="box">
      ...
      <van-tabs v-model="active" color="#000" background="#fff" :sticky="true" @change="tab">
        ...
        <van-tab title="待我处理">
          <Mescroll-vue ref="mescroll" :up="mescrollUp" @init="mescrollInit" class="mescroll">
            ...
            <div class="minBox" v-for="(item,i) in list" :key="i" @click="sign(item.id)">
              <div class="flex col">
                <span>事件编号</span>
        ...
        <van-tab title="我已处理">
          <Mescroll-vue ref="mescroll" :up="mescrollUp" @init="mescrollInit" class="mescroll">
            ...
            <div class="minBox" v-for="(item,i) in list1" :key="i" @click="sign(item.id)">
              <div class="flex col">
                <span>事件编号</span>
                <span>{{item.detailCode}}</span>
              </div>
        ...
        <van-tab title="尚未开始">
          <Mescroll-vue ref="mescroll" :up="mescrollUp" @init="mescrollInit" class="mescroll">
            ...
            <div class="minBox" v-for="(item,i) in list2" :key="i">
              <van-checkbox v-model="item.checked" v-if="checkedFlag" @click="fuXuan(item.id)" />
              <div class="flex col" @click.stop="sign(item.id)">
                <span>事件编号</span>
                <span>{{item.detailCode}}</span>
              </div>
              <div class="flex col" @click.stop="sign(item.id)">
          ...
          <div class="anNiu" v-if="checkedFlag">
            <van-button type="primary" size="large" color="#a48f36">转发</van-button>
          </div>
        </van-tab>
        <van-tab title="已经逾期">
          <Mescroll-vue ref="mescroll" :up="mescrollUp" @init="mescrollInit" class="mescroll">
            <div class="null" v-if="list3.length==0">暂无记录~</div>
            <div class="minBox" v-for="(item,i) in list3" :key="i" @click="sign(item.id)">
              <div class="flex col">
                <span>事件编号</span>
                <span>{{item.detailCode}}</span>
              </div>
    ...
    <div class="break1" v-if="zhuanFa">
      <van-icon name="exchange" size="20" color="rgb(45,126,249)" @click="checkedFlag1" />
    </div>
  </div>
</template>
